...
if (isset($_POST['submit_order'])) {
    ...
    if ($conn->query($orderQuery) === TRUE) {
        $_SESSION['orderConfirmed'] = true; // เก็บสถานะคำสั่งซื้อในเซสชัน
        header("Location: preorder.php"); // รีเฟรชหน้าเพื่อแสดงผลคำสั่งซื้อใหม่
        exit();
    } else {
    ...
    <script>
        ...
        function showPopup() {
            ...
            setTimeout(() => {
                ...
                localStorage.removeItem('orderConfirmed'); // เคลียร์การเก็บสถานะใน localStorage
        ...
        window.onload = () => {
            ...
            if (localStorage.getItem('orderConfirmed')) {
                ...
                localStorage.removeItem('orderConfirmed'); // ลบข้อมูลหลังจากแสดงป็อปอัพแล้ว
            }
        };
